...
- (NSString *)description    {
    return [NSString stringWithFormat:@"Patient details are as follows: \nFirst Name: %@ \nLast Name: %@ \nSSN: %d \nDate of Birth: %@ \nPhone Number: %ld \nStreet Number: %d \nStreet Name: %@ \nAptartment Number: %d \nCity: %@ \nState: %@ \nZip Code: %d \nPrimary Physician: %@", [self firstName], [self lastName], [self ssn], [self dob], [self phoneNumber], [self streetNumber], [self streetName], [self aptNo], [self city], [self state], [self zipCode], [self primaryPhysician]];
}
...
- (void) updateMedicalRecord: (Patient*)p  {
    ...
    MedicalRecord *mr = [[MedicalRecord alloc] init];
