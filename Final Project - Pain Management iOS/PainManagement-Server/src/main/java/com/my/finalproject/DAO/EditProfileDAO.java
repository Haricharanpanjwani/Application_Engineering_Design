...
public class EditProfileDAO extends DAO{
	...
	public String editProfile(EditPatientProfileModel model){
		...
		try{
			...
			profile.setLanguage(model.getLanguage());
			profile.setEnthnonym(model.getEnthnonym());
			profile.setEnthnicity(model.getEnthnicity());
			profile.setCurrentlyWorking(model.getCurrentlyWorking());
			profile.setDate(model.getStartdate());
			profile.setWeight(model.getWeight());			
